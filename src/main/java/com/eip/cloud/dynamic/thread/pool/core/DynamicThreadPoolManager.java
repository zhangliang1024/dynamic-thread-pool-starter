package com.eip.cloud.dynamic.thread.pool.core;

import com.eip.cloud.dynamic.thread.pool.config.prop.DynamicThreadPoolProperties;
import com.eip.cloud.dynamic.thread.pool.enums.QueueTypeEnum;
import com.eip.cloud.dynamic.thread.pool.enums.RejectedExecutionHandlerEnum;
import com.eip.cloud.dynamic.thread.pool.exception.DynamicThreadPoolException;
...
public class DynamicThreadPoolManager {
    ...
    private BlockingQueue getBlockingQueue(String queueType, int queueCapacity, boolean fair) {
        if (!QueueTypeEnum.exists(queueType)) {
            throw new DynamicThreadPoolException("[dynamic pool] - : " + queueType + " queue is not exist");
        }
    ...
    public DynamicThreadPoolExecutor getThreadPoolExecutor(String threadPoolName) {
        ...
        if (threadPoolExecutor == null) {
            throw new DynamicThreadPoolException("[dynamic pool] - : " + threadPoolName + " thread pool is not exist");
        }
